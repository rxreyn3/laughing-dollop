...
def make_api_call_with_retry(func, max_retries: int = 3, initial_delay: float = 1.0, *args, **kwargs) -> Optional[Dict[str, Any]]:
...
def update_conversation_index():
    ...
    try:
        ...
        for channel_id in tqdm(MONITORED_CHANNELS, desc="Processing channels"):
            ...
            try:
                result = make_api_call_with_retry(
                    ...
                    channel=channel_id,
                    oldest=f"{time.time() - 86400 * 10:.6f}"  # Last 10 days, formatted as "1234567890.123456"
                )
                if result is None:
                    print(f"Failed to fetch history for channel {channel_id} after retries")
                    continue
                
                messages = result.get("messages", [])
                for message in tqdm(messages, desc=f"Processing messages in {channel_id}", leave=False):
        ...
        print(f"Indexing complete - Inserted: {inserts}, Updated: {updates} conversations")
